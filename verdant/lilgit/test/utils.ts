...
export function createNotebook(): Notebook {
  return new Notebook({
    rendermime: (NBTestUtils.defaultRenderMime() as unknown) as RenderMimeRegistry,
...
export const DEFAULT_CONTENT: INotebookContent = NBTestUtils.DEFAULT_CONTENT;
...
export function defaultRenderMime(): RenderMimeRegistry {
  return (NBTestUtils.defaultRenderMime() as unknown) as RenderMimeRegistry;
}
...
export async function createMockContext(
  ...
  const manager = (new Mock.ServiceManagerMock() as unknown) as ServiceManager.IManager;
